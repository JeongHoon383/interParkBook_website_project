...
export default function CategoryList_MainSection({ userId }) {
  ...
  useEffect(() => {
    ...
    if (sortField) {
      axios(
        `http://192.168.50.25:9090/category/list/${parameterArr[0]}/${parameterArr[1]}/${parameterArr[2]}/${parameterArr[3]}/${parameterArr[4]}/${parameterArr[5]}/${startIndex}/${endIndex}/${sortField}/${sortOption}/${isSoldout}`
      ).then((result) => {
    ...
    if (userId) {
      axios
        .get(`http://192.168.50.25:9090/wishlist/${userId}`)
  ...
  const handleAddWishlistAll = () => {
    if (userId && checkList.length) {
      if (checkList.length) {
        ...
        axios
          .post("http://192.168.50.25:9090/wishlist/all", wishlistArr)
          .then((result) => {
  ...
  const handleToggleWishlist = (currentBookData, setIsInWishlist) => {
    if (userId) {
      axios
        .post("http://192.168.50.25:9090/wishlist/", {
